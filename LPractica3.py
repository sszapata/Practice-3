...
class GrammarTool(QMainWindow):
    ...
    def generate_trees(self):
        ...
        try:
            ...
            derivation_tree = derivations[0]
            derivation_tree.pretty_print()

            # Generar AST (eliminando nodos no esenciales)
            ast = self.generate_ast(derivation_tree)
            print("Árbol Sintáctico Abstracto:")
            ast.pretty_print()

            self.result_label.setText("Árboles generados correctamente. Consulta la consola para más detalles.")
